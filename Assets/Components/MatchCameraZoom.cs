...
public class MatchCameraZoom : MonoBehaviour {
    ...
    void Start() {
        // Each failure path disables the component so LateUpdate won't fire — preferable
        // to leaving LateUpdate to silently no-op on null refs every frame after a bad setup.
        if (source == null) { Debug.LogError("MatchCameraZoom: source camera not assigned."); enabled = false; return; }
        ...
        if (_assetsPPU == null) { Debug.LogError("MatchCameraZoom: assetsPPU not found on source camera."); enabled = false; return; }

        foreach (var c in GetComponents<Component>()) {
            if (c.GetType() == _srcPpc.GetType()) { _dstPpc = c; break; }
        }
        if (_dstPpc == null) { Debug.LogError("MatchCameraZoom: no matching PixelPerfectCamera on this GameObject."); enabled = false; return; }
    }

    void LateUpdate() {
        // PropertyInfo is cached at Start — GetValue/SetValue here are just method calls
        // on a cached object, not reflection lookups. Negligible cost.
