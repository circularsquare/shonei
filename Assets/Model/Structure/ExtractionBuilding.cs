...
// Extraction buildings produce items from the tile they were built on, at a
// distribution defined by that tile's nExtractionProducts in tilesDb.json (1 liang
// of the base material per craft cycle, plus chance-rolled rare finds). The
// original tile type is captured at placement time — before StructController would
// otherwise replace it — and drives output selection each craft cycle via
// GetExtractionOutputs. Their recipes in recipesDb.json deliberately have empty
// noutputs; the craft hook in AnimalStateManager routes output through this class
// instead. Persisted across save/load by tile name (see SaveSystem.GatherStructure
// / RestoreStructure).
//
// This single class backs BOTH player buildings — the "digging pit" (earth: dirt /
// sand / clay) and the "quarry" (stone: limestone / granite / slate). They differ
// only in JSON data (job, material via the required tile, sprite, research gate,
// cost); the mechanics below are substrate-agnostic. Both set `preservesTile` so
// the tile stays solid and is excavated GRADUALLY (the dish recedes as uses climb),
// finally emptied on depletion (AnimalStateManager.HandleWorking).
//
// ── Dig direction ───────────────────────────────────────────────────────
// The structure digs toward whichever orthogonally-adjacent tile is OPEN, so it
// works in horizontal tunnels and cliff faces, not just from the surface. The
// direction (Up / Left / Right) is chosen ONCE at construction (ChooseDigDirection)
...
// StructureVisualBuilder path) plus a dynamically generated substrate "dish" sprite
// in front. The dish is a 20×20 texture built from the captured tile's interior
// pixels (so it shows limestone, dirt, clay, … as authored) with a parabolic mask
// carved out — thick at the rim, thinnest (the concave bite) toward the open face —
// so it shows the REMAINING substrate as a partially excavated bowl. As uses
// accumulates the remaining substrate recedes away from the opening, and the
// workspot tracks it so the digger appears to stand on the receding floor. On
// depletion the structure is replaced by a regular platform.
//
// The cell's own tile body is suppressed (Tile.bodyRenderSuppressed) while it
// operates, so the carved-away region is a real open hole — the background wall, or
// sky where there is none, shows through the dish's transparent area. `preservesTile`
// is deliberately kept ON: the tile stays solid so NEIGHBOURING tiles still light as
// if it were solid earth/stone. Only this cell's own body is hidden, replaced by the
// dish. The dish's normal map is re-baked from the carved silhouette every time the
// excavation deepens (TileSpriteCache.BakeMaskedNormalMap), so the receding bowl is
// lit as a freshly-cut surface (rim beveled, depths darkened) rather than a flat tile.

// Open face the structure digs toward. Up == 0 so old saves (field absent) default
...
public class ExtractionBuilding : Building, IExtractor {
    ...
    // Called by StructController.Construct before the underlying tile would be emptied.
    // Invalid inputs log an error and leave capturedTile null so the fallback path
    // (recipe.outputs) kicks in — guarantees we never silently produce nothing.
    ...
    // pre-dig state (the door hasn't been wired yet, so no side is bridged through
    // here) and the component ids are current from the last RebuildComponents.
    //
    ...
    // settlement (the nav component most mice occupy) so it doesn't end up mineable
    // only from a sealed cave pocket. Priority: up > a single side > both sides
    // (cosmetic left-tiebreak — both are reachable, so either works).
    void ChooseDigDirection() {
        ...
        if (upReach)                       digDir = DigDir.Up;
            ...
            Debug.LogWarning($"{GetType().Name} at {x},{y}: no open face reachable from the settlement; mining may be impossible.");
        } else {
            // Fully enclosed in solid — degenerate. Dish faces up by convention.
            digDir = DigDir.Up;
            Debug.LogError($"{GetType().Name} at {x},{y}: built fully enclosed (no open face); cannot dig.");
        }
    }
    ...
    // this — after restoring capturedTile via the shared branch — to restore the
    // persisted direction, wire the single door (graph topology — independent of
    // capturedTile, so a structure whose substrate failed to resolve is still
    // reachable rather than orphaned), and rebuild the dish if it can.
    ...
    // Wires the single door on the chosen dig face (the structure's own tile is the
    // door tile; WireDoorEdge resolves the approach). Idempotent. Needs only digDir
    // and workNode (the interior node, repointed by the Structure ctor) — no
    // GameObject, so it's safe on the load path before the dish renders.
    ...
    // Once dug past ~10%, flag the cell so neighbours light their facing edges as
    // exposed cliffs instead of buried seams (Tile.lightAsAir). Below the threshold
    // the cell still reads as solid earth/stone, so a barely-dug hole doesn't pop its
    // neighbours bright. Re-bakes the 3×3 on change only.
    ...
    void UpdateDishTexture() {
        ...
        // from a prior structure, e.g. across scene reloads in PlayMode tests).
        if (substratePixels == null) {
            Sprite substrate = TileSpriteCache.Get(capturedTile.name, ComputeSubstrateCardinalMask(), x, y);
            if (substrate == null) {
                Debug.LogError($"{GetType().Name} dish: TileSpriteCache returned null for '{capturedTile.name}' at {x},{y}");
                return;
            }
        ...
        // At progress=1 the structure is destroyed and replaced with a platform, so the
        ...
        // exposed sides (top/left of a surface hole) get edge lighting while
        // buried sides stay dark — consistent with the surrounding terrain.
    ...
    // the worker visually descends with the hole. workNode here is the interior node
    ...
    void UpdateWorkSpot() {
        ...
        // far wall across the lifetime (matches the dish carve closely enough for the
        // digger to sit on the cut surface).
