...
public class GlobalInventory {
    ...
    // Recipe-aware craftability gate: inputs in stock AND enough (consumable) fuel energy.
    // Crafting is an "always allowed" use — the "consume" flag never blocks an ingredient, so
    // inputs use raw group-aware Quantity. Fuel IS a gated consume channel, so the fuel check
    // stays consumable: this prevents picking a recipe whose only fuel is flagged "don't consume"
    // (PickFuel would then return null and the craft would stall). Every craft-selection site
    // must call this (see Animal.PickRecipe*/ScoreCraftRecipes).
