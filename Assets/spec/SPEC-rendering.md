...
**Tile normal maps** (`TileSpriteCache.BakeNormalMap`): baked per-variant alongside each 20×20 sprite and driven by the sprite's own alpha + an 8-bit adjacency mask (4 cardinals + 4 diagonals). Sprites are keyed by the 4-bit cardinal mask (16 entries per variant); normal maps are keyed by the full 8-bit mask (256 entries) because diagonal openings change the edge-depth alpha at inside corners. For each pixel position we compute an **effective opacity**: interior pixels use their sprite alpha; the four 2×2 overhang corners (e.g. (0–1, 0–1) = BL) are treated as opaque when the matching diagonal neighbour is solid (it spatially owns that corner) and fall back to sprite alpha otherwise; non-corner overhang pixels (strictly one side of the interior) are opaque when the matching cardinal neighbour is solid and fall back to sprite alpha otherwise; positions outside the 20×20 are treated as transparent (empty air). RGB is then an outward-facing bevel normal at every alpha boundary — a pixel whose ±1 cardinal neighbour is effectively transparent gets an outward component in that direction, so e.g. a dithered grass surface correctly lights the row just below any punched-through top pixel. For any rendered pixel the RGB turns out identical across the 16 diagonal sub-variants of a given cardinal mask (diagonals only flip the 2×2 overhang corners, which never sit next to a rendered pixel's cardinal neighbours), so the diagonals effectively only matter for alpha. Alpha is a Euclidean distance-transform to the nearest effectively-transparent pixel, mapped through a smoothstep over `LightFeature.penetrationDepth * TILE` (16 px at default); it drives `LightComposite`'s fade toward `deepAmbient` for underground darkening. This is what lets inside corners (e.g. solid L + solid D + empty BL diagonal) catch light in the interior corner — the transparent BL 2×2 overhang puts an effectively-transparent pixel close enough to the interior corner for the distance transform to reach. Applied via `MaterialPropertyBlock` on tile `SpriteRenderer`s; non-solid tiles use `TileSpriteCache.FlatNormalMap`.
