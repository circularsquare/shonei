...
| 80 | Roads (depth-3 structures) — JSON `sortingOrder: 80, lightingBucket: 1` (raised from 1 to clear the now-high tile bodies). Suppresses the grass overlay on its tile; snow (81) draws over it. |
...
- **Data**: [FlowerType.cs](../Model/FlowerType.cs) loaded from `Resources/flowersDb.json` by `Db.cs`. Each entry has `id`, `name`, `weight`, optional `placement` (`"surface_grass"` default or `"underground"`), and optional `windEffect` (`0..1` scalar attenuating sway amplitude; default `1`, set to `0` for rigid mushrooms / flat moss). Sprite lookup: `Resources/Sprites/Plants/Decorative/<name>.png` — one folder shared by every zone.
...
- **Placement zones**: each tile maps to exactly one zone via `GetZone(tile)`. *Shared rule*: tile is solid, the air tile above exists, has no solid type, and has no `structs[0]` (so buildings / plants take precedence visually).
...
**Wall occlusion (`_PointShadows`)**: when `SettingsManager.pointShadows` is on (every interior mode except the legacy NoWallShadows), `LightCircle.shader` attenuates each lit fragment by the **thickness of solid material the ray to the light crosses**, against the occluder **distance field** (`_OccluderDist`, built by [`OccluderField.cs`](../Lighting/OccluderField.cs)):
...
>
> Additionally `SkyExposure` doesn't flood sky-ambient *into* burrow interior cells (it reads `tile.bodyDrawnByStructure` directly), so a buried burrow reads as a dark den lit only at its sky-open mouth + by torches + through the door. A `debugDraw` toggle renders terrain walls cyan, burrow-perimeter walls magenta. *(History: the sun/sky path was reverted then restored 2026-06-26 — removing it caused a visible regression. There's still an unexplained component to how burrows look; see the propagated-lighting plan.)*
...
6. **Small-blob cull (anti-blink).** A blob is dropped once its diameter falls below `minBlobPixels` (exposed knob, default 2px; `radius·2·pixelsPerUnit < minBlobPixels`). The cloud is crisp-edged pixel art (`edgeSoftness = 0`, hard silhouette), so a blob can only leave by shrinking to nothing or popping — partial-alpha dissolves look out of place and were rejected. As a cell approaches the spawn threshold its `fadeIn` shrinks the radius toward 0; culling only at sub-pixel size means the final vanish is invisible and clusters of fringe blobs *erode* crisply rather than pop. This replaced the old cull at `radius < 0.2·blobRadiusMin` (~4.5px), which chopped off a still-visible disc — and when a humidity-driven threshold sweep pushed a whole cluster's fringe past that cutoff together it read as the cloud "blinking" (chunks ~15px popping out, diagnosed via Monte-Carlo + a live `execute_code` bake-to-bake vanish-detector). A brief detour through per-blob *opacity* dissolve (alpha-fade the edge) was tried and reverted: it requires `edgeSoftness > 0` to produce partial alpha, which blurs the silhouette — wrong for pixel art.
...
3. **Wall type follows the worldgen dirt mask**: `Dirt` where `ComputeDirtMask` set the mask, `Stone` elsewhere (matches what each tile was at fill time — stone-vein passes only convert limestone). With the wavy dirt boundary, cave-roof walls match the dirt pockets above. The type is fixed at world-gen and never changes when the tile is mined or replaced.
