...
5. If density exceeds `Lerp(thresholdClear, thresholdStorm, humidity)`, emit a blob. Radius = `Lerp(blobRadiusMin, blobRadiusMax, excess/excessForMaxSize) × fadeIn × Hash2D(col, row)`. The per-cell random factor scatters sizes uniformly from 0 up to the density-driven target — so a high-noise cluster spawns a fractal-flavoured mix of big and small puffs instead of a row of near-identical lobes (the noise is spatially smooth, so without randomisation neighbouring cells in a dense patch all hit similar excess values and would produce nearly-identical blobs). Each blob also gets a **per-blob aspect** = `blobAspect × (1 + Hash·0.4 − 0.2)` (±20% around the inspector value) so neighbouring blobs don't all look like identical horizontally-stretched ovals. All hashes are deterministic per cell so blobs don't flicker frame-to-frame.
