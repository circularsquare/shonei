...
bucket loops with the chunked tile draw between them. For each bucket `b`,
...
**Single draw (between the bucket loops):**

4. `tileChunkLayer` — `Hidden/ChunkedNormalsCapture` override, alpha = `lerp(0.80, 1.0, _NormalArr.a)` (shadow-caster tier). Samples per-renderer Tex2DArrays via per-vertex slice indices instead of MPB-bound 2D textures. Carries its own `_SortBucket` MPB per chunk (one per-chunk MPB is unavoidable — each chunk binds different Tex2DArray slices), so the bucket-loop global doesn't reach it. Drawn between the loops so its shadow-caster alpha overwrites background's lit-only alpha on overlap (otherwise underground tiles with background behind them lose their edge-depth darkening). See §Tile body rendering.

**Loop B (per bucket)** — tiers drawn after the chunked tile draw:
