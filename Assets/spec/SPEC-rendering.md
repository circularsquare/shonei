...
5. If density exceeds `Lerp(thresholdClear, thresholdStorm, humidity)`, emit a blob. Radius = `Lerp(blobRadiusMin, blobRadiusMax, excess/excessForMaxSize) × fadeIn × Hash2D(col, row)`. The per-cell random factor (`0.5 + 0.5·Hash2D`, i.e. ∈ [0.5, 1]) scatters sizes from half up to the density-driven target — so a high-noise cluster spawns a fractal-flavoured mix of big and small puffs instead of a row of near-identical lobes. The 0.5 floor (rather than a raw [0,1] hash) keeps blobs from collapsing far below the cell spacing, which used to detach a cloud's fading edge row into a string of disconnected dots (the noise is spatially smooth, so without randomisation neighbouring cells in a dense patch all hit similar excess values and would produce nearly-identical blobs). Each blob also gets a **per-blob aspect** = `blobAspect × (1 + Hash·0.4 − 0.2)` (±20% around the inspector value) so neighbouring blobs don't all look like identical horizontally-stretched ovals. All hashes are deterministic per cell so blobs don't flicker frame-to-frame.
6. Density excess below `fadeRange` (= 0.02 above threshold) ramps the parent radius from 0 → full via a `sqrt(t)` curve — smooths popping when noise grazes the threshold (humidity drifting, near-threshold cells crossing) while keeping the tiny phase brief (blob reaches 70% size by quarter-fade, so most of the in-between time it looks close to full).
