...
**Positioning + parallax**: sprite is locked horizontally to `cam.x` (parallax comes from `_NoiseOffset = cam.x · worldLockingX + windOffsetX`, so as the camera or wind moves the noise pattern scrolls within the sprite). Vertically the sprite Y is `cam.y + (bandCenterY − cam.y) · worldLockingY` — i.e. the sprite is centred on `bandCenterY` in world space (with `worldLockingY` interpolating between sky-locked and world-locked). For the asymmetric band — `lyBand = [−halfH·bottomScale, +halfH]` — to fully fit inside the sprite, the sprite needs vertical half-extent ≥ `halfH` (the furthest extreme). At Start, `textureSize.y` is auto-bumped to `(2·halfH + 2·blobRadiusMax) · PPU` if smaller, guaranteeing the upper portion of the band isn't clipped. (Sizing for `halfH` on both sides — even though the band only extends `halfH·bottomScale` down — keeps the sprite's centre at `bandCenterY` so the position math stays the obvious parallax formula, no per-frame offset gymnastics. Costs some unused pixels in the dead zone below the band.) `worldLockingX/Y = 0` = sky-locked (no apparent motion), `= 1` = world-locked (full parallax). Blob sprite-local y = `lyBand` directly.
...
- *Shape*: `textureSize` (256×128 default; `.y` is auto-bumped at Start to `(2·bandHalfHeight + 2·blobRadiusMax) · PPU` if smaller, so the band fits regardless of inspector value), `pixelsPerUnit` (8), `noiseScale` (0.15), `noiseAspect` (horizontal feature stretch, default 2.0).
