...
- **Owner**: [FlowerController.cs](../Controller/FlowerController.cs) — added as a component on `WorldController` from `WorldController.Start`. Holds a `(x, y) → GameObject` dictionary keyed by world coords. Despite the historical name, it handles all decoration zones — surface flowers and underground mushrooms / moss share the same controller because the spawn / eligibility / sway machinery is identical, only the placement rules differ.
...
- **No save state**: spawn is fully deterministic from `(x, y, worldSeed)` via `HashTile`, so on save / load the same layout reappears without persisting anything. New variants added to JSON after a save-was-taken simply re-roll the layout on next load — acceptable for a decorative system.
- **Surface reference**: `World.surfaceY[x]` is the per-column ground-line height — assigned authoritatively by `WorldController.GenerateDefault` from `WorldGen.Generate`'s return value on new worlds, and recomputed in `SaveSystem.ApplySaveData` from the loaded tile grid (not persisted in the save file). Treated as immutable during play — mining the top dirt tile doesn't shift the reference downward, which is intentional so digging a starter pit doesn't redefine "the surface" for decoration purposes. Both this controller AND `OverlayGrowthSystem` consume the same array.
- **Placement zones**: each tile maps to exactly one zone via `GetZone(tile)`. *Shared rule*: tile is solid, the air tile above exists, has no solid type, and has no `structs[0]` (so buildings / plants take precedence visually).
  - **SurfaceGrass**: tile has `type.overlay == "grass"`, `OverlayState.Live`, top-edge overlay bit set (`overlayMask & 0b1000`), not snowed, AND `t.y >= surfaceY[x]` (at-or-above the original ground line). The depth gate keeps flowers strictly outdoor — even if grass somehow spreads onto a tile below the surface line, the flower-spawning step suppresses there so an indoor / cave area never reads as a meadow.
...
- **Reactive lifecycle**: `OnWorldReady` subscribes `cbTileTypeChanged` + `cbOverlayChanged` + `cbSnowChanged` on every tile once. `cbTileTypeChanged` re-evaluates this tile AND the tile below (whose "tile above" check flipped). The overlay / snow callbacks re-evaluate just the tile. Mining a ceiling tile correctly grows new mushrooms on the now-exposed floor below; grass dying / snowing-over drops surface flowers and regrowth respawns the same variant at the same position (deterministic).
...
- **Known gap**: a building placed in the air tile *above* an existing decoration does NOT despawn it automatically — there's no callback on `structs[]` writes. Acceptable for now (rare overlap, decorative-only). If it becomes a problem, hook into `StructController.Place`.
...
`UIChrome.spriteatlas` is currently unrecreated (was overwritten during a
V2 migration attempt). Add a `BuildUIChrome` menu item when needed.
...
5. If density exceeds `Lerp(thresholdClear, thresholdStorm, humidity)`, emit a blob. Radius = `Lerp(blobRadiusMin, blobRadiusMax, excess/excessForMaxSize) × fadeIn × Hash2D(col, row)`, where `fadeIn = clamp01(excess / fadeRange) ^ fadeExponent` ramps the radius 0 → full over the first `fadeRange` of excess above threshold. `fadeExponent < 1` (default 0.8; 0.5 = sqrt) front-loads the growth so fringe cells reach a mergeable size quickly and spend little time tiny — reducing the scatter of disconnected specks at the cloud boundary; the steeper final shrink it implies is hidden by the cull (step 6). The per-cell random factor (`0.5 + 0.5·Hash2D`, i.e. ∈ [0.5, 1]) scatters sizes from half up to the density-driven target — so a high-noise cluster spawns a fractal-flavoured mix of big and small puffs instead of a row of near-identical lobes. The 0.5 floor (rather than a raw [0,1] hash) keeps blobs from collapsing far below the cell spacing (the noise is spatially smooth, so without randomisation neighbouring cells in a dense patch all hit similar excess values and would produce nearly-identical blobs). Each blob also gets a **per-blob aspect** = `blobAspect × (1 + Hash·0.4 − 0.2)` (±20% around the inspector value) so neighbouring blobs don't all look like identical horizontally-stretched ovals. All hashes are deterministic per cell so blobs don't flicker frame-to-frame.
