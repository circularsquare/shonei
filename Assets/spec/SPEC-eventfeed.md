...
- `TradingPanel.HandleCommand` (+ its `Cmd*` handlers) — debug command feedback for `/give`, `/mice [n]`, `/rain`, `/day [n]`, `/wind [v]`, `/timespeed [n]`, `/generate`, `/regenerate`, `/research [id]`, `/online`, `/help` (alias `/commands`). Errors and successes both `Info` (rendered side-by-side in chat); red color tag distinguishes errors visually.
...
- `AnimalStateManager.HandleStuckCheck` — "{name} is stuck" (`Alert`, orange) when a mouse is cut off from the settlement and the auto-rescue snap *also* failed (a genuinely sealed pocket), persisting 5s. The common cut-off cases self-resolve silently via the snap, so this only fires when the player must intervene. See SPEC-systems §Stuck (cut-off) rescue.
- `StructController.CheckReachability` (from the `TickUpdate` usability sweep, ~1s cadence) — "{name} unreachable" (`Alert`, orange) when a workstation's `workNode` is in a nav component no animal shares (unwired door, walled-off pocket, mined-away approach) for longer than an 8s grace. Latched per-building (`Building.unreachableSince` / `unreachableAlerted`) so a persistent fault alerts once and re-arms only after it recovers. Non-workstations and player-disabled buildings are skipped. Building-side analogue of the stuck check.
