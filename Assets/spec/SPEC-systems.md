...
**Brackets share the mount rule** via the same `StructType.sideMounted` flag. A `bracket` is a `solidTop`, depth-1 side-mounted support: it attaches to a solid side like a ladder, but presents a walkable/buildable **top** so platforms, roofs, and buildings can rest on it — enabling cantilevered widening (1-wide base → bracket each side → 3-wide layer above; step out as you go up). Because the wall check only accepts terrain or a **depth-0** building, a bracket can't mount on another bracket (depth 1), so cantilevers extend one tile per level — no chaining, no reach parameter. Brackets are *not* climbable: the ladder-specific `Tile.HasSideLadder*` and nav-refresh paths stay keyed to the literal `ladder_side` name. The player picks the wall side with the mirror toggle (F); there's no edge-hover auto-resolve yet (that's `ladder`-only — see `BuildPanel.ResolveLadderVariant`).
...
2. For any `NeedsMaintenance` structure, decrement `condition` by the per-tick decay rate (clamped at 0). **Sheltered structures decay at half rate**: `IsSheltered(s)` (whole top row covered, probed via `World.IsExposedAbove`) scales the step by `Structure.ShelteredDecayFactor` (0.5). The base step is the *exposed* rate, so a roof overhead is a pure ~2× lifespan upgrade and existing outdoor structures keep their ~30-day calibration. Mirrors `Windmill.HasOpenSky` and the item wet-decay gate — all read the same `IsExposedAbove` blocker (any `solidTop`/`blocksRain` structure or solid tile above).
