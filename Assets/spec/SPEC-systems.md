...
- **LightSource buildings** (torch, fireplace): `LightSource.Update()` calls `Burn` per frame, but *only while lit* (active hour window AND `EnvDarkness() > 0`) — so a torch consumes nothing in daylight. While `building.disabled`, `LightSource.Update()` skips `Burn` and sets `isLit=false`.
...
A greenhouse is an `isGreenhouse` structure sitting at a **foreground depth** (depth 2) so it never contests the plant's `structs[0]` slot — the plant is planted and grows *inside* it. Its footprint tiles back-point to it via `Tile.greenhouse` (set in the `Structure` tile-registration loop, cleared in `Destroy`, mirroring `Tile.interiorBuilding` — so it lands on the gameplay, worldgen, and load paths). A plant whose **anchor tile** is greenhouse-covered reads the four `greenhouse*` tunables off `tile.greenhouse.structType` (SPEC-data) and adjusts four of its growth gates:

- **Temperature (gate 1a)**: instead of testing raw ambient, the gate tests a *regulated* temperature `ambient + (greenhouseTargetTempC − ambient) × greenhouseTempPull` (starter: halfway toward ~26.7 °C/80 °F). Deliberately imperfect — a cold winter day is warmed but not fully neutralised, leaving headroom for stronger greenhouses (`greenhouseTempPull → 1.0`). Heat-sensitive crops can even fall *out* of comfort in high summer, an accepted trade-off. The **season gate is NOT regulated** (herbs are wild-only and can't be planted).
- **Growth rate (gate 1b)**: multiplied by `greenhouseGrowthMult` (starter `1.1`, +10%).
- **Moisture (gates: transpiration + stage cost)**: both the passive per-hour transpiration draw (`MoistureSystem` per-plant pass) and the per-stage advancement cost (`stageMoistureCost` in `Grow`) are scaled by `greenhouseMoistureMult` (starter `0.5`, half). Moisture still drains from the soil tile below as normal — the frame doesn't block rain (it's not `solidTop`/`blocksRain`) — it just consumes less.
...
- **Single door, self-wired**: the JSON declares **no** doors. `ExtractionBuilding` wires exactly one door (the chosen face) via the shared `Structure.WireDoorEdge`/`SuppressDoorRim` helpers, from `OnPlaced` (live) and `RestoreOnLoad` (load) — never the ctor (which runs before the direction is known and would bridge a cave-side approach through the interior node, defeating the reachability check). Rim-suppression teardown is automatic via the base `edgeSuppressTiles` list cleared in `Structure.Destroy`. Up-dug holes additionally open clear L/R faces once mined past 60% (`UpdateSideAccess`); side-dug ones use only their one door.
