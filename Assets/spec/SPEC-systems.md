...
Capacity-1 vertical lifts that A* sees as a single graph edge. Implementation lives in [Elevator.cs](../Model/Structure/Elevator.cs) (which also defines `ElevatorEdgePolicy`); only the load-bearing contracts are documented here.
...
Eight inventory types (`Inventory.InvType`):
...
| Floor | 1 | 1000 fen | 5× normal | Created/destroyed dynamically; one item type per tile |
...
| Market | varies | varies | none | Market building only; created directly as `InvType.Market` in the `Building` constructor |
| Reservoir (Fuel) | 1 | varies | none | Internal building resource (torch wood, foundry coal). No sprite, no tile. See below. |
| Blueprint | varies | varies | none | Construction-material staging on an unbuilt blueprint |
| Furnishing | varies | varies | none | Decorative furnishing slots; track their own per-slot lifetime instead of inventory decay |
...
- **Mixing**: `PutOnFloor` / `ProduceAtTile` prevent different item types from sharing a floor tile (floor inv is single-stack, `n: 1`). `FallItems` moves stacks via `MoveItemTo` rather than `PutOnFloor`, but the single-stack landing inv still holds only one type — mismatched items that can't fit are subtracted from GlobalInventory and warned (see above).
...
Each animal has five `InvType.Equip` inventory instances (1 stack each, registered with InventoryController for GlobalInventory tracking, no sprite, decay at normal rate). All five are saved/restored (`WorldSaveData.*SlotInv`, null-safe gather/load in SaveSystem) and dropped/destroyed on death:

| Slot | Field | Capacity | Purpose |
|------|-------|----------|---------|
| Food | `foodSlotInv` | 500 fen (5 liang) | Carries food for eating |
| Tool | `toolSlotInv` | 200 fen | Equipped tool (work speed bonus) |
| Clothing | `clothingSlotInv` | 200 fen | Body slot — equipped clothing (temperature comfort bonus) |
...
| Hat | `hatSlotInv` | 200 fen | Head slot — separate from clothing, so a mouse can wear a hat AND a shirt. See §Hats. |

**UI (gear grid)**: equip slots render as an RPG-style 2×3 box grid via the shared `EquipGrid` component (`Components/`) — `EquipSlotDisplay` boxes in a `GridLayoutGroup`, not text. Both the InfoPanel quick view (`AnimalInfoView`) and the population panel detail pane (`MouseDetailView`) hold an `EquipGrid` and call `Bind(animal)` / `Refresh()`; the layout is code-defined (`EquipGrid.layout`) — `food/hat/book` on top, a reserved blank cell + `top/tool` below. Empty slots draw outline placeholders from `Resources/Sprites/Misc/equipslots/` (box frame = `equipslotbox`); the box prefab is `Resources/Prefabs/EquipSlotBox.prefab`. Each box hovers a tooltip: worn gear (tool/clothing/hat/book) shows `condition NN%` (1 − decayCounter/(unitFen×maxDecayCount)), food shows amount. All slots always show (no reachability gating). Adding a slot = one `EquipGrid.layout` entry + a placeholder sprite.
...
**Hats** (`hatSlotInv`, head slot): `Db.hatItems` = leaves under the `"hat"` group. Hats are a **hard profession uniform** — a mouse wears ONLY its `job.preferredHat` (resolved at load to `job.preferredHatItem`); a job with no preferred hat wears nothing. Enforced on three paths:
- `FindHat()` only ever fetches `preferredHatItem` (an equip candidate in `ChooseTask` at `EquipUrgency` when the slot is empty). Skips if consumption is disabled for that hat.
- `SetJob()` drops a now-mismatched hat immediately on a job change (including switching to a hatless job).
- A `ChooseTask` self-heal drops a mismatched hat on the next idle tick — covers loads and saves written before the rule existed.

**Hat bonuses** (data-driven per hat in `itemsDb.json`, applied in `ModifierSystem`):
- `skillBonus` (parsed to `skillBonusEffect`) + `skillBonusLevels`: a worn hat whose `skillBonusEffect == taskSkill` adds `skillBonusLevels × SkillSet.BonusPerLevel` into the skill multiplier in `GetWorkMultiplier` — so it speeds that skill's work AND research (research rides `workEfficiency`).
- `walkBonus`: multiplies travel speed in `GetTravelSpeedMultiplier` (`speed × (1 + walkBonus)`).

**Hat overlay**: `AnimationController` draws the worn hat from `hatSlotInv` on the Head child renderer; `MouseHeadIcon` reflects it in UI.
...
- **Books** (equipDecayRate 1.0, set on the `books` group → inherited by every book): a tech book wears as a scientist studies with it in `bookSlotInv` (research runs in the Working state, so `ApplyEquipDecay` ticks). Fiction reading is *leisure*, not Working, so the fiction book wears only via its passive `decayRate` 0.3. Crafting time was cut to compensate (book recipe `duration` 120 → 100).
- **Food** doesn't set `equipDecayRate` — it has its own consumption mechanic (eating).
...
Distinct from liquid `tile.water` — moisture represents damp **soil**. Lives on `Tile.moisture` as a `byte` and is only meaningful on **solid** tiles (dirt/stone). Air tiles keep their moisture at 0 by convention; all moisture sweeps skip non-solid tiles. Drives plant growth gating; see SPEC-data.md `plantsDb.json` for the per-plant comfort-range fields.

**Per-material capacity** (`TileType.moistureCapacity`, default 100 = `MoistureSystem.MoistureMax`): the max moisture a material holds. Earth (dirt/sand/clay) = 100; limestone = 50; granite/slate = 20. Set in `tilesDb.json`. It does triple duty — the clamp ceiling, the diffusion conductivity (see below), and the well pull threshold (`cap/2`). All clamps read `tile.type.moistureCapacity`; `MoistureMax` is retained only as the full/default/pool capacity (greenhouse + pot pools stay cap-100 abstractions). Plant comfort is still **absolute** (a no-op today since every plant roots on cap-100 soil/pools); fractionalizing it is deferred to the `per-material-moisture-capacity` plan, to land with the first sub-100-cap-rooted plant (moss).
...
- **Water-neighbour seep** (`MoistureSystem.SeepPerSecond()`): each solid tile with moisture headroom picks its wettest 4-orthogonal water neighbour and absorbs up to `MoistureSeepMoisturePerSec` (currently 10) moisture from it. Yield is `MoisturePerWaterUnit` (10) moisture per sim water unit drained, **derived** from `MoisturePerLiang` (50) — the canonical moisture ⇄ water rate: 1 liang of water ≡ 50 soil-moisture (water is measured in liang; sim water units are an internal fluid-display detail). The well reservoir reads the derived per-unit value (it stores water in sim units); farmer watering reads `MoisturePerLiang` directly. Both are **public** for these consumers (see §Farmer watering and §Well below).

  *Amortisation*: because `tile.water` is integer, the source can't lose a fractional unit per tick directly; the cost is amortised via a per-soil-tile debt accumulator (`_seepDebt`) that holds absorbed-but-unpaid moisture and cashes out 1 water from the source whenever it exceeds `MoisturePerWaterUnit`. If the source runs dry, absorption caps so leftover debt stays below one whole water unit (no free moisture).

  Net effect: dry soil still saturates from a wet neighbour in ~10 s real-time (rate unchanged); with `MoisturePerWaterUnit == MoistureSeepMoisturePerSec` the source drains ~1 water/s per absorbing soil tile — a 1-tile pond feeding 4 solid neighbours empties in ~40 s under load. Pump-irrigated farms stay wet only while the pump keeps refilling water.
...
- **Soil-to-soil diffusion** (all solid tiles): the driving force is the **saturation** gap, not the absolute one. A tile pulls `MoistureDiffusionPctPerHour`% (currently 8) toward `target = max over solid neighbours of (nbrMoisture × thisCap ÷ nbrCap)` — the moisture it would hold at its most-saturated neighbour's saturation (`MaxSaturationTarget`). That `target` form bakes the **puller-cap conductivity scaling** in: a low-cap tile (slate, cap 20) pulls slowly (≈1.6%/h) and is a barrier; the dirt past it pulls fast (8%) from the slate. One-way (never lowers), non-conservative (source not debited). **Gravity bias**: the rate is nudged ±`MoistureGravityBiasPct` (currently 3) by the winning neighbour's vertical direction — down-flow (source above) ≈11%/h, up-flow (source below) ≈5%/h, horizontal 8%/h — so rain percolates down fast while groundwater wicks up slowly. It biases the *rate*, not the equilibrium (a connected body still equalizes to equal saturation), and applies to all soil, not just per-material caps. It equalizes **saturation**, so the far side of a low-cap barrier can fully saturate — just throttled by the slow trickle through it (a speed-bump, not a hard ceiling; evaporation + plant draw keep the far side dry in practice). Sub-unit pulls accumulate in a per-tile fixed-point debt buffer (`_diffDebt`, 1/100-moisture) that cashes out whole units — essential here, since cap-scaled rates round to zero without it. **No-op when all caps are 100** (`target == maxNeighbour`). Approximates capillary spread — a water-adjacent stone wall's moisture slowly propagates inward, a rained-on surface row slowly wets the column below.
...
**Worldgen seed**: `WorldGen.SeedMoisture` sets every solid tile to `StartingMoisture = 90` (clamped to the tile's `moistureCapacity`, so low-cap stone seeds lower) at world generation so plants can grow from turn 1 (set high so a fresh world that hasn't rained yet still supports crops). Surface soil then drifts from this baseline under rain/decay; underground holds unless a water neighbour bumps it higher.
...
- **Exchange rate** (`MoistureSystem.MoistureForWaterFen` / `WaterFenForMoisture`): both the water item and soil agree in liang (100 fen = 1 liang), and `1 liang ≡ MoisturePerLiang (50) moisture`, so the conversion is direct: `moisture = fen × 50 / 100`. The seep physics yields the same per-liang rate by construction (`MoisturePerWaterUnit` derives from `MoisturePerLiang`), so the watering and world economies can't drift apart.
...
`WildHerbSystem` skips greenhouse-covered tiles when scattering wild herbs/lilies, and skips any candidate tile already holding a structure **at any depth** (`AnyStruct` — not just `structs[0]`), so a herb/lily never visually overlaps a building, platform, road, shaft, or bridge. `FlowerController` applies the same all-depths rule to decorative flowers.
...
When balancing a plant's `[moistureMin, moistureMax]` comfort window against `growthTime`, model soil moisture as a four-phase cycle (dry drain → dry floor → rain ramp → rain cap) driven by the rain humidity walk in `WeatherSystem`. Compute `happyFraction` = hours-inside-comfort-window per cycle / total cycle hours. Since the moisture gate is now soft (out-of-window grows at `DroughtGrowthRate = 0.3`, not 0), effective speed ≈ `happyFraction + 0.3 × (1 − happyFraction)`, and effective grow time ≈ `growthTime / that`.
...
A structure built into a solid tile (burrow, with `requiredTileName: "dirt"`; well, with `digsSolidColumn`) can opt out of the default "convert footprint to empty" mining behaviour by setting `preservesTile: true` in JSON. The footprint tiles keep their original type — grass continues, snow accumulates, water still blocked, support unchanged — and the structure renders in front of them as if it were a carved hole.
...
The flag is generic — any future "hole into a bank" building can opt in by pairing it with `requiredTileName`, `requiresSolidTilePlacement`, or `digsSolidColumn` (the well's variable-height column variant — see §Well below). Schema is documented in SPEC-data.md.
...
### Well (groundwater reservoir + bucket draw)

A 1-wide shaft dug straight DOWN into solid ground (soil or stone) by a digger, resized with Q/E at placement (`shapes`, ny 3..8). Like the burrow it sets `preservesTile`, so the shaft tiles stay SOLID — the surrounding terrain keeps clean buried edges and the interior reads dark, with the `well_s` facade (`_b`/`_m`/`_t` slices) drawn in front. It's the first **shaped** `preservesTile` building, which is why the backdrop loop is shape-aware. Introduces three generic flags (`digsSolidColumn`, `anchorAtTop`, `suppressOverlay` — see SPEC-data.md).

- **Geometry**: the bottom `ny-1` tiles are the shaft (dug, solid, water cells); the top tile is the open surface wellhead — a standable workstation with `solidTop` OFF (can't stand/build on top; its standability comes from the solid shaft below). `workTile` is overridden to the wellhead; placement grows DOWNWARD (`anchorAtTop`).
- **Why a reservoir, not `tile.water`**: clean carved edges need solid neighbour tiles (the chunk bake keys on `type.solid`) but the natural water sim only renders in NON-solid tiles — mutually exclusive. So the well keeps its tiles solid and stores water as a single int `storedWater` (tile-water units, `0..Capacity = (ny-1)*WaterMax`), rendered via the **container-liquid path** (`Building.TryGetDisplayLiquid` + a `waterPixelOffsets` zone synthesised in code across the whole shaft column, since the `{name}_w` mask scan only covers the anchor tile). Same Water shader as ponds → identical look. See `reference_burrow_edges_vs_simwater` memory.
- **Groundwater economy** (`Well.Tick`, per second via `Well.TickAll` in `World.Tick`, after the generic moisture pass): for each shaft tile's solid orthogonal neighbours (+ the floor + the well's own preserved-solid footprint column, so it taps the soil it sits in, not just its borders), pull soil moisture into the reservoir when `moisture > neighbourCap/2`, seep back out when below it (the pivot scales with the neighbour's `moistureCapacity`, so a well works in low-cap stone — granite cap 20 pivots at 10, not an unreachable 50); gradual (`MoistureFlowPerNeighbourPerSec = 1`, kept gentle to roughly match the tile-diffusion feel) with a sub-unit `_moistureDebt` accumulator. The generic `MoistureSystem.SeepPerSecond` does NOT fight it — there's no `tile.water` on the solid shaft to drain.
- **Conversion rates** (single source of truth, all in `WaterController` / `MoistureSystem`): `WaterController.WaterMax = 160` is the internal **sim resolution** of a full tile (sub-pixel, for flat settling — NOT an economic quantity); `WaterController.LiangPerFullTile = 32` is the **economic value** of a full tile ⟹ full tile = **32 liang**. There is deliberately no "units per liang" ratio constant — sim-unit ⇄ item conversions go through `WaterController.WaterUnitsToFen` / `LiangToWaterUnits` (derived from those two). Moisture ⇄ water is authored as `MoistureSystem.MoisturePerLiang = 50` (1 liang of water ≡ 50 soil-moisture ⟹ a saturated soil tile, moisture 100 = **2 liang**); the integer-granular seep/well drains use the **derived** `MoisturePerWaterUnit` (= `MoisturePerLiang × LiangPerFullTile / WaterMax` = 10 moisture per sim unit), which `Well` references rather than keeping its own copy.
- **Bucket draw** (hauler): registered as a craft workstation with a `well`-tile recipe (output `water`, 5 liang) so it competes for water demand like the pump, but `Animal.ChooseCraftTask` branches to a bespoke **`DrawWaterTask`** (not `CraftTask`). The hauler walks to the wellhead; the Well's per-frame draw state machine (`AdvanceDraw`, driven by the `WellBucket` component) lowers the bucket to the water surface → soaks ~1s → raises it → produces ~5 liang into the hauler and drains `storedWater`, then a `DropObjective` hauls it off. Time scales with water depth (emptier = deeper, slower). Gated on `HasDrawableWater` (`storedWater > LiangToWaterUnits(MinDrawableLiang=10)`). The bucket + rope are child GOs; the rope is a tinted solid quad (colour/width sampled from `well_rope.png`) scaled to length, so it neither tile-gaps nor stretches and its bottom tracks the bucket 1:1.
- **Persisted**: `storedWater` (`StructureSaveData.wellStoredWater`); depth rides the already-saved `shapeIndex`. Files: `Well.cs`, `DrawWaterTask`/`DrawWaterObjective`, `WellBucket`.

## Mounted structures — side and ceiling (`ladder_side`, `bracket`, `torch_side`, `lantern`)

A mount hangs on a surface in an air tile (no floor needed), keyed on the `StructType.mountTo` enum (`None`/`Side`/`Ceiling`, never the name) so placement, suspension, and support-mine cleanup all generalise across members. **Side** mounts attach to a vertical wall beside the tile — `dir = mirrored ? +1 : -1` puts the wall on the opposite side from the sprite's lean. **Ceiling** mounts (`lantern`) hang from the solid underside of the tile above (mirror-invariant). The two faces share one placement branch and one support predicate; they differ only in which neighbour and which face they probe.

**Edge-hover variant resolution.** A base build type with a `sideVariant` field (`ladder` → `ladder_side`, `torch` → `torch_side`) auto-swaps to its side-mounted variant when the cursor hovers near a tile's left/right edge (`BuildPanel.ResolveSideVariant`, called from `MouseController`). The cost panel and hotkeys keep showing the base type; only the placed structure switches. `torch_side` (a wall-mounted `torch`: same fuel/light, `defaultLocked`, no own build button) is reached this way — select `torch`, hover an edge. `bracket` and `lantern` have no `sideVariant` and are placed directly via their own button. The `lantern` is a **ceiling-mounted hanging torch** — identical fuel/light/ember setup to `torch`, plus a `paper` cost, placed in an air tile under a ceiling.

**Mounting rule** (`StructPlacement.MountSurfacePresent`, the single source of truth shared by the `st.isMounted` placement branch and `Blueprint.IsSuspended`): the support surface must be either a natural/built **solid tile**, OR a **building whose sprite actually has body on the facing edge**. Rejected against a plant and against a building's visually-empty footprint tile (e.g. a windmill's blade tiles, claimed in `structs[0]` but drawing nothing). Side mounts probe the wall tile's left/right face; ceiling mounts probe the above-tile's **bottom** face.

- "Has body on that edge" is decided by `StructType.EdgeSolid(dx, dy, MountFace, mirrored)`, which reads per-footprint-tile left/right/**bottom** edge **bitmasks** baked from the building sprite's alpha. Baked offline by the **`Tools > Bake Building Edge Solidity`** editor command ([BuildingEdgeMaskBaker.cs](../Editor/BuildingEdgeMaskBaker.cs)) into `Resources/buildingEdgeMasks.json`, loaded at `Db` startup. Probing at gameplay/load time was ~15–25 ms (over budget); baking drops it to ~0. **Re-run the bake after editing any building sprite** — the masks are a stale-able artifact, not auto-refreshed.
- The edge test samples the **outer 2 pixel lines** of a face (columns for left/right, rows for bottom; ≥50% of the perpendicular 16 px opaque), so a frame inset by 1px (e.g. the workshop) still counts as solid. Buildings the baker can't map cleanly (animated sheets, shape variants, sprite size ≠ `nx·16 × ny·16`) get no entry → `EdgeSolid` returns permissive (per-face: an unbaked `-1` mask treats that edge as solid) so legit mounts are never wrongly blocked. A pre-bottom-bake `buildingEdgeMasks.json` (no `bottom` field) loads bottom as `-1`, so ceiling mounts work permissively until re-baked.
...
**Brackets** are a `solidTop`, depth-1 side-mount: they attach to a solid side like a ladder, but present a walkable/buildable **top** so platforms, roofs, and buildings can rest on them — enabling cantilevered widening (1-wide base → bracket each side → 3-wide layer above; step out as you go up). Because the surface check only accepts terrain or a **depth-0** building, a mount can't attach to another mount (depth 1), so cantilevers extend one tile per level — no chaining, no reach parameter. Brackets, side torches, and lanterns are *not* climbable: the ladder-specific `Tile.HasSideLadder*`, nav-refresh, and `LadderPolicy` paths stay keyed to the literal `ladder_side` name.

**Mining the support drops its mounts** (`Blueprint.DestroyDependentMounts`, fired from `Complete` when `minesTile`): when a tile is mined to empty (the `"empty"` action or a `requiresSolidTilePlacement` building like the mineshaft), any mount that depended on the now-removed surface is instantly `Destroy()`-ed — side-mounts leaning on it from either side (`Tile.GetSideMount(dir)`) and a ceiling-mount hanging directly below it (`Tile.GetCeilingMount()`), both scanning all depths. Its half-cost materials are appended to the blueprint's `pendingOutput`, so they flow to the **mining mouse** via `animal.Produce` (which falls back to a floor drop, then vanish-with-log, if the mouse can't hold them).
...
**Rain/wind**: `WeatherSystem` advances weather per in-game hour from `World.Update`. Rain is driven by a continuous `humidity` field — a slow Ornstein-Uhlenbeck walk reverting toward `humidityMean`; rain starts when humidity crosses `rainThreshold` from below and stops when it crosses back (intensity is a smooth function of humidity, see `ComputeWetness`). Lighting hooks (sun/ambient multipliers) are polled by `SunController` each frame. While raining, `OnHourElapsed` runs:
