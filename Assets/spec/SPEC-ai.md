...
Each satisfaction decays exponentially each SlowUpdate (`×0.9044`). Score = sum over satisfied needs (≥1.0 threshold) — `+1` each, except `"alcohol"` which is `+2` (a satisfied tipple is worth more than a tidy bench) — plus housing (bool) + temperature (−1/5°C to +2). `Db.happinessMaxScore` = need count + 1 (housing) + 2 (temp max) + 1 (alcohol's extra point, added when the need is registered).
