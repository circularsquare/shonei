...
| 130–139 | production — clothing | weaver, tailor, hattery |
...
| `category` | string | UI build menu group: `"tiles"`, `"structures"`, `"production"`, `"storage"`, `"housing"` (plant types are auto-routed to `"plants"` via `isPlant`). Any other value hides the building from the build menu. |
| `defaultLocked` | bool? | hidden from build menu until researched |
| `mountTo` | string? | `"side"` or `"ceiling"` (default ground-supported) — hangs in an air tile on a surface instead of resting on the ground, via the mount-attach branch in `StructPlacement`, skipping standability. `side` = a vertical wall, player picks the side via mirror (F) (`ladder_side`, `bracket`, `torch_side`); `ceiling` = the underside of the tile above (`lantern`). Mining the support drops the mount (`Blueprint.DestroyDependentMounts`). See SPEC-systems.md §Mounted structures. |
...
| `preservesTile` | bool? | opt out of the tile-to-empty swap in `StructController.Construct()`. The footprint tiles keep their original type (grass still grows, snow accumulates, water stays blocked, support unchanged), and the structure renders in front of them as if it were a hole carved out. Yield still fires: `Blueprint.Complete` captures each footprint tile's `tile.type.products` into `pendingOutput`. The backdrop + clear (`Structure.BuildPreservedTileBackdrop` / `Destroy`) iterate the chosen **shape** column, so a variable-height `preservesTile` building (well) is fully covered, not just its base `ny`. Used by `burrow` (a hole into a dirt bank — multi-tile, yields 3× dirt) and `well`. Pair with `requiredTileName`, `mustBeSolidTile`, or `digsSolidColumn`. |
...
| `digsSolidColumn` | bool? | a 1-wide structure dug straight DOWN into solid ground for its full chosen-shape height: the bottom `ny-1` tiles must be solid + mineable (the shaft), the top tile must be open (the surface workstation). Generalises `mustBeSolidTile`/`requiresSolidTilePlacement` (anchor-only) to a variable-height column — validated per-tile in `StructPlacement.GetPlacementFailReason`, and `OnDeserialized` also sets `requiresSolidTilePlacement` so the empty-tile/support exemptions + stone-mining tech gate apply. Used by `well`. |
| `anchorAtTop` | bool? | cursor maps to the TOP tile instead of the bottom anchor, so a downward-dug shaped structure places its surface piece under the cursor and Q/E extends the shaft DOWNWARD. `MouseController` offsets the placement anchor by `-(ny-1)` (feeds both the preview and the blueprint); everything downstream still uses the bottom (dy=0) anchor. Used by `well`. |
| `suppressOverlay` | bool? | the structure draws its own facade over its footprint, so the surface grass/dirt overlay must NOT render on its tiles (it draws at order 80, over the facade). Unlike the burrow — which keeps grass growing over its dug top — a well's sky-exposed shaft mouth would otherwise sprout grass. Same effect roads get via their depth-3 layer; checked in `TileMeshController.BuildOverlayGeometry` (render) + `OverlayGrowthSystem` (growth). Used by `well`. |
...
| `emissionStrength` | float? | self-emission (the glowing `_e`/`_f` body pixels) multiplier, independent of the radial light reach (default 1; `<1` dims the glow). Feeds `LightSource.emissionMult` → LightFeature's per-emitter `_EmissionScale`. Use to make a soft-glow building (lantern, `0.5`) read dimmer than a hot flame on the same global `_EmissionStrength`. |
...
| `greenhouseTargetTempC` | float? | interior temp (°C) the frame pulls ambient toward each growth tick (default `25`). NOTE: authored in **Celsius** like all temps. |
...
ID ranges (a guide for picking a collision-free id for a new item — not every leaf is listed; `itemsDb.json` is authoritative):
...
| 10–19 | raw stone group + leaves (stone=10, slate=11, granite=12, limestone=13, gypsum=14) |
| 20–29 | earth items (dirt=20, sand=21, clay=22) |
| 30–39 | `ores` group=33 (iron ore=31, coal=32, malachite=34, cassiterite=35) |
| 40–49 | `metal` group=40 (iron bar=41, copper bar=42, tin bar=43, bronze bar=44); `molten` group=45 (molten copper=46, tin=47, bronze=48, glass=49) — `hidden` foundry intermediaries |
| 50–59 | glass=50, clay mold=51 |
...
| 210–249 | liquids and processed food (soymilk=230, tofu=241, cheese=242, yeast=245, rice wine=246, `drinks` group=247) |
| 250–259 | fiber group (ramie) |
| 260–269 | cloth group (ramie cloth) |
| 270–279 | clothing group (ramie shirt); `hat` group=275 (bamboo hat=276, straw hat=277, cloth hat=278) |
| 280–299 | misc materials/furnishings (bamboo=280, stools=290) |
| 300+ | books — `book` group=300, `fiction_book`=301, then **runtime** tech books from 302 up (see the books note below) |
...
| 420–429 | `tonics` group=420 (vigor=421, warming=422, cooling=423, sleep=424) — see `Item.buffType` |
...
| `skillBonus` | string? | hat bonus: names a `Skill` domain (parsed to `Item.skillBonusEffect` in `OnDeserialized`). A worn hat grants `skillBonusLevels` extra effective levels to that skill — read by `ModifierSystem` (1 level = `SkillSet.BonusPerLevel` = +5%). |
| `skillBonusLevels` | int? | effective skill levels the worn hat adds to `skillBonus`'s domain |
| `walkBonus` | float? | hat bonus: fractional travel-speed boost while worn (`0.02` = +2%); read by `ModifierSystem`. |
...
| `discrete` | bool? | stored/moved in whole-unit (`unitFen`) multiples only (e.g. tools, clothing); inherited by children |
| `unitWeight` | float? | discrete items only: weight (in liang) of one whole unit, which sets `unitFen` (the discrete step size *and* storage bulk — weight and bulk are deliberately one number). Absent/0 → 1 liang/unit. Lets a stool/statue cost >1 liang per unit. |
| `itemClass` | enum? | `"default"` (solid), `"liquid"` (water, soymilk), `"book"` (tech/fiction books). Storage inventories match class exactly — liquids only fit in tanks, books only in bookshelves. Inherited by children. Defaults to `"default"`. **A non-default leaf must sit under a same-class group (or at top level).** The storage allow-tree (`StoragePanel`) parents each item row under its group's GameObject and hides the group row when the group's class mismatches the storage — Unity then forces the group's children inactive too, so a `liquid` leaf buried under a `default` group is unreachable in a tank even though the leaf itself qualifies. This is why drinks (soymilk, rice wine) form their own `liquid` group rather than living under `food`. |
...
| `startDiscovered` | bool? | revealed in the inventory/storage trees from game start — no research or production needed (e.g. `water`, drawn from ponds). Absent → item appears only once first produced/discovered. |
| `hidden` | bool? | never shown in any item-list UI (inventory panel, market holdings, storage allow trees) even after production — for internal intermediaries that aren't tradable/usable as-is (the foundry's molten metals/glass). Discovery and recipe use still work; only display rows are suppressed. Cascades to children. |
| `furnishingSlot` | string? | marks this item installable into a house's `FurnishingSlots`; matched against the slot's `slotNames[i]` to decide fit. Null = not a furnishing. Cascades to descendants. |
| `furnishingHappiness` | float? | flat happiness added to every resident of the house while this item is installed in a slot. |
| `furnishingLifetimeDays` | float? | in-game days the furnishing lasts; counts down each tick while installed and empties the slot on 0-cross. Independent of `decayRate`. |
| `furnishingSprite` | string? | sprite name under `Resources/Sprites/Buildings/furnishings/` overlaid on the house tile while installed. Null = no visual. |
| `furnishingCost` | float? | amount consumed to install one furnishing (authored in liang). 0/absent → one whole unit (discrete) or 1 liang. Resolved to `furnishingCostFen` (floored to a `unitFen` multiple for discrete items; a sub-unit cost floors to 0 → un-installable, logged by Db validation). |
...
**ID-band collision gotcha:** the runtime tech-book band (302+) **grows as research nodes are added**, so hand-authored items must stay clear of it — a clash logs a "multiple items with id" error and one item silently overwrites the other. This is why hats sit at 275–278, not 310+.
...
| Field | Type | Notes |
|-------|------|-------|
| `duration` | float | seconds to complete one batch. UNTENDED = elapsed in-game seconds (temperature-scaled); TENDED = seconds of worker labour. Large for slow ferments (rice wine `960` = 2 in-game days × `ticksInDay`). Displayed via `Recipe.FormatDuration` — under half an in-game day (`< dayLen × 0.5`, ~240s default) as seconds (`"8s"`), else in-game days (`"2 days"`). |
...
| `preferredHat` | string? | item name of the hat this job's mice seek to wear (farmer→bamboo, hauler→straw, scientist/scribe→cloth). Resolved to `Job.preferredHatItem` at load; absent → no hat sought. |
...
| `nremovalProducts` | `[{name, quantity}]`? | what removing/uprooting the plant yields (liang), as distinct from harvesting. Null → defaults to `nproducts`, so removal yields a harvest's worth unless overridden (e.g. apple tree → `wood`). |
| `fractionalRemoval` | bool? | when true, removing the plant *before* full maturity still yields `nremovalProducts`, scaled down by growth (`Plant.RemovalYield`). Wood trees set this; crops leave it false so an immature crop yields nothing on removal. |
...
| `moistureDrawPerHour` | float? | passive draw from the soil tile below each in-game hour (default 2) |
...
| `growthFrames` | `string[][]`? | optional data-driven growth table: one entry per growth stage, each listing the sprite cell for every occupied tile bottom→up (e.g. `["b4","g0"]` = a 2-tile stage). When present it fully overrides the implicit "4 stages/tile" model — `maxStage`, per-stage height, and sprites all come from the table (lets fruit trees author a bespoke base→canopy→flower→fruit sequence on a fixed height). Null → legacy formula. |
| `fruitCycleStages` | int? | fruit-tree behaviour. 0 (default) = harvest fells the plant (reset to stage 0, release upper tiles). >0 = harvest rewinds this many stages and leaves the tree standing, so only the final fruiting sub-cycle replays (rewind target = `maxStage − fruitCycleStages`, which should be the first fruitless mature frame). |
| `fruitRotTicks` | int? | fruit-tree only: a ripe (max-stage) tree left unharvested this many ticks drops its fruit (no yield) and rewinds to re-fruit, so it never sits ripe forever. Ticks (matching `growthTime`). 0 = never rots. |
...
`PlantType` extends `StructType`, so a plant entry also accepts the shared StructType fields — notably `displayName`, `description`, and `capacity` (simultaneous harvesters, always 1 for plants) — documented in the buildingsDb table above.
...
| `moistureCapacity` | int | max soil moisture this material holds (default **100** = `MoistureSystem.MoistureMax`; omit for earth). Stone holds less: limestone/limestone_placed **50**, granite/slate **20**. Drives the moisture clamp, the diffusion conductivity (rate scales with cap — low-cap stone is a slow barrier), and the well pull threshold (`cap/2`). See SPEC-systems "Soil Moisture". |
