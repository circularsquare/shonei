...
public class Astar
{
    ...
    private Node[] Successors(int x, int y, bool[,] grid, int rows, int cols)
    {
        ...
        int W = x - 1;
        if (x < 0 || x > cols - 1){
            Debug.Log("nav successor x out of range");
            return null;
        }
        if (y < 0 || y > rows - 1){
            Debug.Log("nav successor y out of range");
            return null;
        }
    ...
    {
        bool[,] arr = new bool[grid.GetLength(0), grid.GetLength(1)];
    ...
    public static void FlipArray(bool[,] grid){
    ...
    public Astar(bool[,] grid, Vector2Int s, Vector2Int e, Type type = Type.Manhattan)
    {
        ...
        if (s.x >= cols){
            Debug.Log("navigation start x out of range!");
            s.x = cols - 1;
        } 
        if (s.x < 0){
            Debug.Log("navigation start x out of range!");
        ...
        if (s.y >= rows){
            Debug.Log("navigation start y out of range!");
            s.y = rows - 1;
        } 
        if (s.x < 0){
            Debug.Log("navigation start y out of range!");
        ...
        if (e.x >= cols){
            Debug.Log("navigation end x out of range!");
            e.x = cols - 1;
        } 
        if (e.x < 0){
            Debug.Log("navigation end x out of range!");
        ...
        if (e.y >= rows){
            Debug.Log("navigation end y out of range!");
            e.y = rows - 1;
        } 
        if (e.x < 0){
            Debug.Log("navigation end y out of range!");
